...
integer function AHI_Main_Read(filename,ahi_data2,n_bands,band_ids,do_not_alloc,do_geo) result(status)
	...
	do i=1,n_bands
		if(band_ids(i)>0 .and. band_ids(i).lt.HIMAWARI_NCHANS)ahi_main%inchans(band_ids(i))	=	1
	enddo	
	...
	if (ahi_main%ahi_data%memory_alloc_d.ne.1) then
		retval	=	AHI_alloc_vals(ahi_main)
!		retval	=	AHI_alloc_vals_data(ahi_main%ahi_data,n_bands)
	...
	retval	=	AHI_Setup_Read_Chans(ahi_main)
	if (retval.ne.HIMAWARI_SUCCESS) then
		status	=	HIMAWARI_FAILURE
		return
	endif
	if (do_geo.eq.1) then
		retval	=	AHI_Pix2Geo(ahi_main)
		if (retval.ne.HIMAWARI_SUCCESS) then
			status	=	HIMAWARI_FAILURE
			return
		endif
		retval	=	AHI_calc_satangs(ahi_main)
		if (retval.ne.HIMAWARI_SUCCESS) then
			status	=	HIMAWARI_FAILURE
			return
		endif	
		retval	=	AHI_Calctime(ahi_main)
		if (retval.ne.HIMAWARI_SUCCESS) then
			status	=	HIMAWARI_FAILURE
			return
		endif
...
integer function AHI_Setup_Read_Chans(ahi_main) result(status)
	...
	do i=1,HIMAWARI_NCHANS
		if (ahi_main%inchans(i)==1) then
			...
			do j=minseg,maxseg
...
#endif
				if (j.eq.0) then
					retval	=	AHI_get_file_name(i,ahi_main%ahi_info%timeslot,ahi_main%ahi_info%satname,ahi_main%ahi_info%indir,fname)
				else
					retval	=	AHI_get_file_name_seg(i,j,ahi_main%ahi_info%timeslot,ahi_main%ahi_info%satname,ahi_main%ahi_info%indir,fname)
				endif
				if (retval.ne.HIMAWARI_SUCCESS) then
				...
				retval	=	AHI_readchan(fname,tdata2(:,:),i,ahi_main%convert(i),ahi_main%ahi_navdata,j,segpos(j))
...
#endif
			retval	=	AHI_resample_hres(tdata2, ahi_main%ahi_data%indata(:,:,bandpos),xsize,ysize)
			if (retval.ne.HIMAWARI_SUCCESS) then
...
integer function AHI_readchan(fname, indata,band,convert,ahi_nav,seg,sline)result(status)
	...
	if (band.lt.7) then
		retval			=	AHI_readhdr_VIS(filelun,ahi_hdrvis)
		...
		retval			=	AHI_readhdr_IR(filelun,ahi_hdrir)
...
integer function AHI_resample_hres(indata, outdata,xsize,ysize) result(status)
...
integer function AHI_SavetoNCDF(outdata,fname,bname,newfile) result(status)
